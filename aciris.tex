...
Sherpa provides several $\chi^{2}$ statistics with different variance definitions. 
...
statistics select one of the several $\sigma_{i}^{2}$ formula, 
listed in Table~\ref{table:statistics}
% in the Fitting Tool, including a least-squares
%option and .

Also included as fit statistics options are two maximum likelihood functions, Cash and 
C-statistic, which are based on Poisson statistics \citep{1979ApJ...228..939C}. 
The statistic formula are listed in Table~\ref{table:statistics}.

\begin{table}
\caption{\textbf{Sherpa fit statistics.} The fit statistic names as they appear in Iris 
are under \textit{Statistic} and corresponding formulae are under \textit{Formula}. 
$D_{i}$ is the $y$-data, $M_{i}$ is the predicted model value, and $\sigma_{i}^{2}$ 
is the variance of $D_{i}$.}
