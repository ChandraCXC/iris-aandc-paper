...
where $D_{i}$ is the observed data, $M_{i}$ is the predicted model, and $\sigma_{i}^{2}$ 
is the variance of $D_{i}$. Users wanted to use $\chi^{2}$ statistics choose  
one of the $\sigma_{i}^{2}$ definitions in Table~\ref{table:statistics}.

Also included as fit statistics options are two maximum likelihood functions, Cash and 
C-statistic, which are based on Poisson statistics. The statistic formula are listed 
